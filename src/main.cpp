...
uint256 hashGenesisBlock("0x00000e136054fbebac46684d39d70a175f96ced711ef3463b27ab138e6a3110e"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // CryptoBit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "CryptoBit Signed Message:\n";
...
{
    // CryptoBit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    int64 nSubsidy = 0; 
    if(nHeight >= 1000) {
        if((nHeight >= 4000 && dDiff > 45) || nHeight >= 9000) { // GPU/ASIC difficulty calc
            // 3333333/(((x+2600)/9)^2)
            nSubsidy = (7777777.0 / (pow((dDiff+2600.0)/10.0,3.0)));
            if (nSubsidy > 25) nSubsidy = 90;
            if (nSubsidy < 5) nSubsidy = 50;
        } else { // CPU mining calc
            nSubsidy = (77777.0 / (pow((dDiff+51.0)/70,3.0)));
            if (nSubsidy > 100) nSubsidy = 700;
            if (nSubsidy < 5) nSubsidy = 70;
        }
    } else {
        nSubsidy = (7777.0 / (pow((dDiff+1.0),2.0)));
        if (nSubsidy > 100) nSubsidy = 700;
    ...
    for(int i = 250000; i <= nHeight; i += 250000) nSubsidy *= 1.00;

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // CryptoBit: 1 day
static const int64 nTargetSpacing = 60; // CryptoBit: 1 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // CryptoBit: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, cryptobit - DarkGravity v2, written by Evan Duffield - evan@cryptobit.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, cryptobit - DarkGravity v3, written by Evan Duffield - evan@cryptobit.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // CryptoBit: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // CryptoBit: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Cryptobit";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1406265594;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2557073;
        ...
        assert(block.hashMerkleRoot == uint256("0x0c90af937a928bdf5e1c297add7740eadac56b07aeacad271b8d6698a3bb1871"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptoBitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptoBitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptoBitMiner(CWallet *pwallet)
{
    printf("CryptoBitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryptobit-miner");
    ...
    try { loop {
        ...
        printf("Running CryptoBitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptoBitMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptoBitMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - CRYPTOBIT **********
    Copyright 2014, Bitblock Developers 
        eduffield - evan@cryptobit.io
        InternetApe - kyle@cryptobit.io
*/
